...
-- ─── CONFIG SYSTEM (deobfuscated arch port) ───────────────────────────────
local CFG_FOLDER   = "ArchConfig"
local CFG_HTTP     = HttpService
local _cfgWidgets  = {}   -- widget refs for SetValue on load
local _cfgSelected = "default"

-- ── helpers ──────────────────────────────────────────────────────────────
local function _safeIsfile(p)
    return type(isfile) == "function" and pcall(isfile, p) and isfile(p)
end
local function _safeIsfolder(p)
    return type(isfolder) == "function" and pcall(isfolder, p) and isfolder(p)
end
local function _ensureFolder()
    if type(makefolder) == "function" and not _safeIsfolder(CFG_FOLDER) then
...
local function _listConfigs()
    local out = {"default"}
    if type(listfiles) ~= "function" or not _safeIsfolder(CFG_FOLDER) then return out end
...
-- ── Color3 serialize / deserialize ───────────────────────────────────────
local function _c3Enc(c)
    if typeof(c) == "Color3" then return {r=c.R, g=c.G, b=c.B} end
    return nil
end
local function _c3Dec(t)
...
-- ── build save payload from S ─────────────────────────────────────────────
local function _buildPayload(configName)
    local d = {}
    for k, v in pairs(S) do
        local t = typeof(v)
        if t == "Color3" then
            d[k] = {__type="Color3", r=v.R, g=v.G, b=v.B}
        elseif t == "EnumItem" then
            d[k] = {__type="Enum", enum=tostring(v.EnumType), name=v.Name}
        elseif t ~= "function" and t ~= "RBXScriptConnection" and t ~= "Instance" and t ~= "userdata" then
            d[k] = v
        end
    end
    d["__configName"] = configName
    d["__autoLoad"]   = (_cfgWidgets.autoLoad and true or false)
    return d
end
...
local function _saveConfig(name)
    ...
    if type(writefile) ~= "function" then
        Library:Notify("writefile not available", "warning"); return false
    end
    _ensureFolder()
    local ok, enc = pcall(function()
        return CFG_HTTP:JSONEncode(_buildPayload(name))
    end)
    if not ok or type(enc) ~= "string" then
        Library:Notify("Encode failed: " .. tostring(enc), "warning"); return false
    end
    local ok2, err = pcall(writefile, _cfgPath(name), enc)
    if ok2 then
...
-- ── LOAD ─────────────────────────────────────────────────────────────────
local function _loadConfig(name, silent)
    if type(name) ~= "string" or name == "" then name = "default" end
    local path = _cfgPath(name)
    if not _safeIsfile(path) then
        if not silent then Library:Notify("Config '" .. name .. "' not found!", "warning") end
        return false
    end
    local ok, raw = pcall(readfile, path)
    if not ok or type(raw) ~= "string" then
        if not silent then Library:Notify("Read failed", "warning") end; return false
    end
    local ok2, data = pcall(function() return CFG_HTTP:JSONDecode(raw) end)
    if not ok2 or type(data) ~= "table" then
        if not silent then Library:Notify("Parse failed", "warning") end; return false
    end

    -- apply to S with type reconstruction
    for k, v in pairs(data) do
        if k == "__configName" or k == "__autoLoad" then
        elseif type(v) == "table" then
            if v.__type == "Color3" then
                S[k] = Color3.new(
                    math.clamp(v.r or 0, 0, 1),
                    math.clamp(v.g or 0, 0, 1),
                    math.clamp(v.b or 0, 0, 1)
                )
            elseif v.__type == "Enum" then
                local eType = v.enum and Enum[v.enum]
                if eType then
                    local ok3, eVal = pcall(function() return eType[v.name] end)
                    if ok3 and eVal then S[k] = eVal end
                end
            else
                S[k] = v
            end
    ...
    if data.__autoLoad ~= nil and _cfgWidgets.autoLoad then
        pcall(function() _cfgWidgets.autoLoad.Set(data.__autoLoad) end)
    end

    if not silent then Library:Notify("Config '" .. name .. "' loaded!", "success") end
    return true
end

-- ── DELETE ────────────────────────────────────────────────────────────────
local function _deleteConfig(name)
    if type(name) ~= "string" or name == "" or name == "default" then
        Library:Notify("Cannot delete default config!", "warning"); return false
    end
    local path = _cfgPath(name)
    if not _safeIsfile(path) then
        Library:Notify("Config '" .. name .. "' not found!", "warning"); return false
    end
    if type(delfile) ~= "function" then
        Library:Notify("delfile not available", "warning"); return false
    end
    local ok, err = pcall(delfile, path)
    if ok then
        Library:Notify("Config '" .. name .. "' deleted!", "success")
        _cfgSelected = "default"
        return true
    end
...
local PresetsGroup = ConfigTab:Group("Presets")
...
ConfigGroup:Textbox({
    ...
    Callback = function(v)
        if type(v) == "string" and v ~= "" then _cfgSelected = v end
    end
})
...
ConfigGroup:Button({
    Name = "Save Config",
    Variant = "Primary",
    Callback = function() _saveConfig(_cfgSelected) end
})
ConfigGroup:Button({
    Name = "Load Config",
    Callback = function() _loadConfig(_cfgSelected, false) end
})
ConfigGroup:Button({
    Name = "Delete Config",
    Callback = function() _deleteConfig(_cfgSelected) end
})
ConfigGroup:Button({
    Name = "List Configs",
    Callback = function()
        local configs = _listConfigs()
        Library:Notify("Configs: " .. table.concat(configs, ", "), "success")
    end
})

-- auto-load toggle
_cfgWidgets.autoLoad = ConfigGroup:Toggle({
    Name = "Auto Load on Start",
    Default = false,
    Tooltip = "Auto loads 'default' config on startup",
    Callback = function(v) end
})

-- ── PRESETS ───────────────────────────────────────────────────────────────
local function _applyPreset(data, label)
    for k, v in pairs(data) do S[k] = v end
    ApplyAllSettings()
    if S.undergroundActive and type(_G.SetUndergroundDepthExt) == "function" then
        _G.SetUndergroundDepthExt(S.undergroundDepth)
    end
    Library:Notify(label .. " preset loaded", "success")
end
...
PresetsGroup:Button({Name = "Legit",  Callback = function() _applyPreset(_legitPreset, "Legit") end})
PresetsGroup:Button({Name = "Rage",   Variant = "Danger", Callback = function() _applyPreset(_ragePreset, "Rage") end})

-- ── auto-load on startup ──────────────────────────────────────────────────
task.defer(function()
    if _cfgWidgets.autoLoad then
        local autoOn = false
        pcall(function() autoOn = _cfgWidgets.autoLoad.Enabled end)
        if autoOn and _safeIsfile(_cfgPath("default")) then
            _loadConfig("default", true)
        end
    end
end)
