...
-- ============================================================
-- Kicia Ragebot / Flickbot integration
-- Added as a separate tab immediately before the Settings tab.
-- ============================================================
do
    local KiciaServices = {
    ...
    local KPlayer = KiciaServices.Players.LocalPlayer
    local KReplicatedStorage = KiciaServices.ReplicatedStorage
    local KWorkspace = KiciaServices.Workspace
    local KRunService = KiciaServices.RunService
    local KUserInputService = KiciaServices.UserInputService
    local KCollectionService = KiciaServices.CollectionService

    local clonerefK = cloneref or clonereference or function(i) return i end
    local clonefunctionK = clonefunction or copyfunction or function(f) return f end
    local sethiddenpropertyK = sethiddenproperty or function(obj, prop, val) pcall(function() obj[prop] = val end) end
    local getthreadidentityK = getthreadidentity or function() return 0 end
    local setthreadidentityK = setthreadidentity or function() end
    ...
    local function KResolveEnumLibrary()
        local mod = KReplicatedStorage:FindFirstChild("Modules") and KReplicatedStorage.Modules:FindFirstChild("EnumLibrary")
        if mod then
    ...
    local KEnumLibrary = KResolveEnumLibrary()
    local KFighterController
    local function KResolveFighterController()
        if KFighterController then return KFighterController end
        local scripts = KPlayer:FindFirstChild("PlayerScripts")
        ...
        if mod then
            ...
            if ok and type(ctrl) == "table" then KFighterController = ctrl; return ctrl end
        end
        return nil
    end

    local function KResolveLocalFighter()
        local ctrl = KResolveFighterController()
        if not ctrl then return nil end
        if ctrl.LocalFighter then return ctrl.LocalFighter end
        if type(ctrl.GetFighter) == "function" then
            local ok, f = pcall(ctrl.GetFighter, ctrl, KPlayer)
            if ok and f then return f end
        end
        return nil
    end

    local function KReadValue(obj, key)
        if not obj then return nil end
        if type(obj.Get) == "function" then
            local ok, v = pcall(obj.Get, obj, key)
            if ok then return v end
        end
        return obj[key]
    end

    local function KIsEnemyPlayer(player)
        if player == KPlayer then return false end
        local a, b = KPlayer:GetAttribute("TeamID"), player:GetAttribute("TeamID")
        if a and b then return a ~= b end
        if KPlayer.Team and player.Team then return KPlayer.Team ~= player.Team end
        return true
    end

    local function KIsInvincible(entity)
        local data = entity and entity.Data
        return type(data) == "table" and data.IsInvincible == true
    end

    local function KCollectEnemies()
        local out = {}
        local ctrl = KResolveFighterController()
        ...
        for _, fighter in ipairs(objects) do
            ...
            if player and player ~= KPlayer and KIsEnemyPlayer(player) then
                ...
                if model then
                    ...
                    if head and root then
                        ...
                        table.insert(out, {
                            fighter=fighter, player=player, entity=entity, model=model,
                            hitboxHead=head, rootPart=root, humanoid=hum,
                            alive=hum == nil or hum.Health > 0,
                            invincible=KIsInvincible(entity),
                        })
                    end
                end
            end
    ...
    local function KIsValidTarget(e)
        return e and e.alive and not e.invincible
    end

    local function KSelectTarget()
        local enemies = KCollectEnemies()
        if KSettings.PrioritizeHackers then
            for _, e in ipairs(enemies) do
                if e.hacker and KIsValidTarget(e) then return e end
            end
        end
        for _, e in ipairs(enemies) do
            if KIsValidTarget(e) then return e end
        end
        return nil
    end

    local function KIsPointOOB(pos)
        local buffer = 4
        for _, safe in ipairs(KCollectionService:GetTagged("OutOfBoundsSafePart")) do
            if safe:IsA("BasePart") then
                ...
                if math.abs(rel.X)<=hx and math.abs(rel.Y)<=hy and math.abs(rel.Z)<=hz then return false end
            end
        end
        for _, oob in ipairs(KCollectionService:GetTagged("OutOfBoundsPart")) do
            if oob:IsA("BasePart") then
                local rel=oob.CFrame:PointToObjectSpace(pos)
                local hx,hy,hz=oob.Size.X/2,oob.Size.Y/2,oob.Size.Z/2
                if math.abs(rel.X)<=hx+buffer and math.abs(rel.Y)<=hy+buffer and math.abs(rel.Z)<=hz+buffer then return true end
            end
        end
        return false
    end

    local function KItemCategory(idx)
        if idx==1 then return "Primary" elseif idx==2 then return "Secondary" elseif idx==3 then return "Melee" end
    end
    local function KCategoryEnabled(cat)
        if cat=="Primary" then return KSettings.WeaponPrimary elseif cat=="Secondary" then return KSettings.WeaponSecondary elseif cat=="Melee" then return KSettings.WeaponMelee end
        return false
    end

    local function KGetAction(fighter)
        local items=fighter and fighter.Items
        if type(items)~="table" then return nil end
        local onEmpty=KSettings.OnEmpty
        ...
        for slotKey,item in next,items do
            if type(item)=="table" then
                local cat=KItemCategory(slotKey)
                if cat and KCategoryEnabled(cat) then
                    anyEnabled=true
                    local prio=1
                    local info=item.Info or {}
                    local itType=info.Type or "Gun"
                    local ammo=KReadValue(item,"Ammo")
                    if itType=="Gun" and (ammo==nil or ammo==0) then
                        local reserve=KReadValue(item,"AmmoReserve")
                        if reserve and reserve>0 then
                            if onEmpty=="Reload" then
                                if prio<bestAttackPri then bestAttack,bestAttackPri,bestAttackIdx=item,prio,slotKey end
                            else
                                if prio<bestSwapPri then bestSwap,bestSwapPri,bestSwapIdx=item,prio,slotKey end
                            end
                        end
                    elseif bestAttack==nil or prio<bestAttackPri then
                        bestAttack,bestAttackPri,bestAttackIdx=item,prio,slotKey
                    end
                end
            end
        end
        ...
        if bestAttack==nil then
            ...
            if bestSwap.IsEquipped then return {type="Reload",item=bestSwap,index=bestSwapIdx} end
        ...
        local ammo=KReadValue(bestAttack,"Ammo")
    ...
    local KNEG=-9e37
    local function KBuildAim(base,pitch,oy,oz)
        return {["\0"]=base["\0"],["\1"]=base["\1"],["\2"]=base["\2"],["\3"]=pitch,["\4"]=oy,["\5"]=oz}
    end
    local KAIM_ABOVE_ORIGIN={["\0"]=KNEG,["\1"]=0,["\2"]=0}
    local KAIM_ABOVE_END={["\0"]=0,["\1"]=-90000000,["\2"]=0}
    local KAIM_EXTRA={["\0"]=0,["\1"]=1,["\2"]=0,["\3"]=0,["\4"]=0,["\5"]=0}
    local KPITCH_ABOVE=-math.pi/2

    local KCleanRemote=Instance.new("RemoteEvent")
    local KFireServer=clonefunctionK(KCleanRemote.FireServer)
    local function KUseItemRemote()
        local rem=KReplicatedStorage:FindFirstChild("Remotes")
        local rep=rem and rem:FindFirstChild("Replication")
        local fighter=rep and rep:FindFirstChild("Fighter")
        return fighter and fighter:FindFirstChild("UseItem")
    end
    local function KFireGun(objectId,isRaycast,aim1,aim2,head,extra)
        local remote=KUseItemRemote(); if not remote or not KEnumLibrary then return end
        local token=KEnumLibrary:ToEnum("StartShooting"); if not token then return end
        local inner={["\0"]=aim1,["\1"]=aim2,["\2"]=head,["\3"]=extra}
        local payload=isRaycast and {["\1"]=inner,["\2"]=true} or {["\1"]=inner}
        KFireServer(remote,objectId,token,payload,nil)
    end
    local function KFireMelee(objectId,a,b,c,d)
        local remote=KUseItemRemote(); if not remote or not KEnumLibrary then return end
        local token=KEnumLibrary:ToEnum("StartShooting"); local anim=KEnumLibrary:ToEnum("AttackAnimation1")
        if token and anim then KFireServer(remote,objectId,token,{["\1"]={["\0"]=a,["\1"]=b,["\2"]=c,["\3"]=d},["\2"]=anim},nil) end
    end
    local function KFireReload(objectId)
        local remote=KUseItemRemote(); if not remote or not KEnumLibrary then return end
        local start=KEnumLibrary:ToEnum("StartReloading"); local reload=KEnumLibrary:ToEnum("Reload")
        if start and reload then KFireServer(remote,objectId,start,{["\1"]=reload,["\2"]=reload},nil) end
    end
    local function KItemObjectId(item)
        local data=item and item.Data
        return data and data.ObjectID
    end
    local function KEquipItem(item,index)
    ...
    local function KReloadItem(item)
        local reserve=KReadValue(item,"AmmoReserve") or 0
        local ammo=KReadValue(item,"Ammo") or 0
        local maxAmmo=KReadValue(item,"Info.MaxAmmo") or 0
        if reserve>0 and maxAmmo>ammo then KFireReload(KItemObjectId(item)) end
    end

    local KRootDesync={}; KRootDesync.__index=KRootDesync
    function KRootDesync.new(root)
        local self=setmetatable({_rootPart=root,_oldCFrame=root.CFrame,_serverCFrame=nil},KRootDesync)
        self._bind="KiciaRootDesync_"..tostring(root:GetDebugId())
        pcall(function() KRunService:BindToRenderStep(self._bind,Enum.RenderPriority.First.Value,function()
            if self._oldCFrame then self._rootPart.CFrame=self._oldCFrame; self._oldCFrame=nil end
        end) end)
        return self
    end
    function KRootDesync:SetServerCFrame(cf) self._serverCFrame=cf end
    function KRootDesync:GetClientCFrame() return self._oldCFrame or self._rootPart.CFrame end
    function KRootDesync:HeartbeatUpdate() if self._serverCFrame then self._oldCFrame=self._rootPart.CFrame; self._rootPart.CFrame=self._serverCFrame end end
    function KRootDesync:Destroy() if self._bind then pcall(KRunService.UnbindFromRenderStep,KRunService,self._bind) end end

    local KPartGlue={}; KPartGlue.__index=KPartGlue
    function KPartGlue.new() return setmetatable({_bindings={}},KPartGlue) end
    function KPartGlue:Acquire(ourPart,hitboxPart)
        local prev=getthreadidentityK(); pcall(setthreadidentityK,8)
        sethiddenpropertyK(ourPart,"PhysicsRepRootPart",hitboxPart); pcall(setthreadidentityK,prev)
        self._bindings[ourPart]=hitboxPart
        local cf=CFrame.new(math.random(-100000,-10000),100000,math.random(-100000,10000)); hitboxPart.CFrame=cf
        return CFrame.new(cf.Position)
    end
    function KPartGlue:Free(ourPart)
        if self._bindings[ourPart] then
            self._bindings[ourPart]=nil
            local prev=getthreadidentityK(); pcall(setthreadidentityK,8); sethiddenpropertyK(ourPart,"PhysicsRepRootPart",nil); pcall(setthreadidentityK,prev)
        end
    end

    local KViewDriver={}; KViewDriver.__index=KViewDriver
    function KViewDriver.new() return setmetatable({_slots={},_winning=nil},KViewDriver) end
    function KViewDriver:Send(priority,angles)
        self._slots[priority]=angles
        local best=nil
        for pri,val in pairs(self._slots) do if val and (best==nil or pri>best) then best=pri end end
        self._winning=best and self._slots[best] or nil
    end
    function KViewDriver:Flush()
        local w=self._winning; if not w then return end
        local rem=KReplicatedStorage:FindFirstChild("Remotes")
        local rep=rem and rem:FindFirstChild("Replication")
        local f=rep and rep:FindFirstChild("Fighter")
        local remote=f and f:FindFirstChild("UpdateCameraRotation")
        if remote then
            local pitch,yaw=w.X,w.Y
    ...
    local KCharacterController={}; KCharacterController.__index=KCharacterController
    function KCharacterController.new(root) return setmetatable({_rootDesync=KRootDesync.new(root),_viewDriver=KViewDriver.new()},KCharacterController) end
    function KCharacterController:SetServerCFrame(cf) self._rootDesync:SetServerCFrame(cf) end
    function KCharacterController:GetClientCFrame() return self._rootDesync:GetClientCFrame() end
    function KCharacterController:SendViewAngles(priority,angles) self._viewDriver:Send(priority,angles) end
    function KCharacterController:HeartbeatUpdate() self._rootDesync:HeartbeatUpdate(); self._viewDriver:Flush() end
    function KCharacterController:Destroy() self._rootDesync:Destroy() end

    local function KRandomEvade(clientCF)
        local base=KSettings.RandomBaseRadius; local factor=KSettings.RandomRadiusFactor
        local angle=math.random()*2*math.pi; local r=base+math.random()*base*factor
        local anchor=KSettings.RandomAnchorFromCharacter and clientCF.Position or Vector3.new(0,clientCF.Position.Y,0)
        return CFrame.new(anchor+Vector3.new(math.cos(angle)*r,0,math.sin(angle)*r))
    end
    local function KTranslocateEvade()
        local oobs=KCollectionService:GetTagged("OutOfBoundsPart"); local chosen=oobs and oobs[1]
        if chosen then return chosen.CFrame*CFrame.new(0,-chosen.Size.Y/2+KSettings.TranslocateOffset,0) end
        return CFrame.new(math.random(-100000,100000),100000,math.random(-100000,100000))
    end
    local function KProjectileEvade(clientCF) return CFrame.new(clientCF.Position+Vector3.new(0,20,0)) end

    local KRageController={}; KRageController.__index=KRageController
    function KRageController.new() return setmetatable({_enabled=false,_partGlue=KPartGlue.new(),_charCtrl=nil,_boundRoot=nil},KRageController) end
    function KRageController:SetEnabled(v) self._enabled=v; if not v then self:Reset() end end
    function KRageController:EnsureCharacterController()
        local char=KPlayer.Character; local root=char and char:FindFirstChild("HumanoidRootPart")
        if not root then if self._charCtrl then self._charCtrl:Destroy() end; self._charCtrl=nil; self._boundRoot=nil; return nil end
        if not self._charCtrl or self._boundRoot~=root then if self._charCtrl then self._charCtrl:Destroy() end; self._charCtrl=KCharacterController.new(root); self._boundRoot=root end
        return self._charCtrl
    end
    function KRageController:Reset()
        if self._charCtrl then self._charCtrl:SetServerCFrame(nil); self._charCtrl:SendViewAngles(20,nil); self._charCtrl:HeartbeatUpdate() end
        if self._boundRoot then self._partGlue:Free(self._boundRoot) end
    end
    function KRageController:Destroy() self:SetEnabled(false); self:Reset(); if self._charCtrl then self._charCtrl:Destroy(); self._charCtrl=nil end end
    function KRageController:Update()
        local fighter=KResolveLocalFighter(); local cc=self:EnsureCharacterController()
        if not fighter or not cc or not self._enabled then self:Reset(); return end
        local root=self._boundRoot; if not root then return end
        local target=KSelectTarget(); local action=KGetAction(fighter); local clientCF=cc:GetClientCFrame()
        if target and action then
            local voidCF=self._partGlue:Acquire(root,target.hitboxHead); cc:SetServerCFrame(voidCF); cc:SendViewAngles(20,nil)
        else
            local ev=KSettings.EvasionMode=="Random" and KRandomEvade(clientCF) or (KSettings.EvasionMode=="Translocate" and KTranslocateEvade() or KProjectileEvade(clientCF))
            if not KIsPointOOB(ev.Position) then cc:SetServerCFrame(ev) else cc:SetServerCFrame(clientCF) end
            self._partGlue:Free(root)
        end
        if action and action.type=="Attack" and target then
            local item=action.item; local oid=KItemObjectId(item); local info=item.Info or {}
            if oid and info.Type=="Gun" then
                KFireGun(oid,info.IsRaycast==true,KBuildAim(KAIM_ABOVE_ORIGIN,KPITCH_ABOVE,0,0),KBuildAim(KAIM_ABOVE_END,KPITCH_ABOVE,0,0),target.hitboxHead,KAIM_EXTRA)
            elseif oid and (info.Class=="Melee" or info.Type=="Melee") then
                KFireMelee(oid,KBuildAim(KAIM_ABOVE_ORIGIN,KPITCH_ABOVE,0,0),KBuildAim(KAIM_ABOVE_END,KPITCH_ABOVE,0,0),target.hitboxHead,KAIM_EXTRA)
            end
        elseif action and action.type=="Reload" then KReloadItem(action.item)
        elseif action and action.type=="Swap" then KEquipItem(action.item,action.index) end
        cc:HeartbeatUpdate()
    end

    local KController=KRageController.new()
    local KFlick={State="Idle",Trajectory=nil,TrajectoryIndex=1,ElapsedMs=0,TotalMs=0,Selected=nil,Timer=0,Rng=Random.new()}
    local function KNormal(rng,mean,std)
        local u1=math.max(rng:NextNumber(),1e-15); local u2=rng:NextNumber()
        return mean+std*math.sqrt(-2*math.log(u1))*math.cos(2*math.pi*u2)
    end
    local function KGamma(rng,shape,scale)
        local factor=1
        if shape<1 then factor=rng:NextNumber()^(1/shape); shape=shape+1 end
        local adj=shape-1/3; local root=math.sqrt(9*adj)
        while true do
            local sample=KNormal(rng,0,1); local candidate=(1+sample/root)^3
            if candidate>0 then
                local u=math.max(rng:NextNumber(),1e-15)
                if u<1-0.0331*sample^4 or math.log(u)<0.5*sample^2+adj*(1-candidate+math.log(candidate)) then return adj*candidate*scale*factor end
            end
        end
    end
    local function KCdf(t,mean,sigma)
        if t<=0 then return 0 end
        local x=(math.log(t)-mean)/sigma
        return 0.5*(1+math.erf(x/math.sqrt(2)))
    end
    local function KBump(v) if v<=0 or v>=1 then return 0 end; return v^2*(1-v)^3/0.03456 end
    local function KGenerateTrajectory(sx,sy,tx,ty)
        local rng=KFlick.Rng; local dx,dy=tx-sx,ty-sy; local dist=math.sqrt(dx^2+dy^2); if dist<1 then return {} end
        local dirX,dirY=dx/dist,dy/dist; local angle=math.atan2(dy,dx); local duration=KSettings.FlickbotDuration
        local hum=KSettings.FlickbotHumanness/30; local curv=KSettings.FlickbotCurvature/1000
        local primaryDist=dist*(0.97+rng:NextNumber()*0.03); local peakRatio=0.29+rng:NextNumber()*0.06
        local mean=math.log(duration*peakRatio)+0.04; local sigma=0.2
        local curvature=dist*curv*(0.5+0.8*math.abs(math.sin(angle))); local tremorFreq=8+rng:NextNumber()*4; local tremorAmp=(0.05+rng:NextNumber()*0.13)*hum
        local nx,ny=0,0; local times={}; local t=0; local endTime=duration*1.15
        while t<endTime do t=t+KGamma(rng,3.5,7.8/3.5); if t<=endTime+15 then times[#times+1]=t end end
        local traj={}
        for i,timeMs in ipairs(times) do
            local progress=KCdf(timeMs,mean,sigma); local x=sx+dirX*primaryDist*progress-dirY*curvature*KBump(progress); local y=sy+dirY*primaryDist*progress+dirX*curvature*KBump(progress)
            local delta=i>1 and (timeMs-times[i-1])/1000 or 0.01; local ouTheta=3.5; local ouSigma=0.5*hum
            nx=nx*(1-ouTheta*delta)+ouSigma*math.sqrt(math.max(delta,0))*KNormal(rng,0,1); ny=ny*(1-ouTheta*delta)+ouSigma*math.sqrt(math.max(delta,0))*KNormal(rng,0,1)
            local tremor=1/(1+primaryDist*0.3); local txn=math.sin(2*math.pi*tremorFreq*timeMs/1000)*tremorAmp*tremor; local tyn=math.sin(2*math.pi*tremorFreq*timeMs/1000+1)*tremorAmp*tremor
            traj[#traj+1]={x=x+nx+txn,y=y+ny+tyn,t=timeMs}
        end
        for i=1,#traj do
            local a=traj[i-1] or traj[i]; local b=traj[i+1] or traj[i]; local vx=b.x-a.x; local vy=b.y-a.y; local mag=math.sqrt(vx^2+vy^2); traj[i].direction=Vector3.new(vx/mag,vy/mag,0) end
        return traj
    end
    local function KUpdateFlickbot(dt)
        if not KSettings.FlickbotEnabled then KFlick.State="Idle"; return end
        local key=KSettings.FlickbotKeybind; local held=key and key~=Enum.KeyCode.Unknown and KUserInputService:IsKeyDown(key)
        if held and KFlick.State=="Idle" then
            local cam=KWorkspace.CurrentCamera; local target=KSelectTarget()
            if cam and target then
                local pt,on=cam:WorldToViewportPoint(target.rootPart.Position)
                if on then
                    local traj,Kvp=KGenerateTrajectory(cam.ViewportSize.X/2,cam.ViewportSize.Y/2,pt.X,pt.Y),cam.ViewportSize
                    if #traj>1 then KFlick.Trajectory=traj; KFlick.TrajectoryIndex=1; KFlick.ElapsedMs=0; KFlick.TotalMs=traj[#traj].t; KFlick.Selected=target; KFlick.State="Flicking" end
                end
            end
        end
        if KFlick.State=="Flicking" then
            local tr=KFlick.Trajectory; if not tr then KFlick.State="Idle"; return end
            KFlick.ElapsedMs=KFlick.ElapsedMs+dt*1000; local idx=KFlick.TrajectoryIndex
            while idx<#tr and tr[idx+1].t<=KFlick.ElapsedMs do idx=idx+1 end
            KFlick.TrajectoryIndex=idx; local cur=tr[idx]; local nxt=tr[idx+1]
            if not nxt then
                if KSettings.FlickbotShot then
                    local delay=KSettings.FlickbotShotDelay/1000
                    task.delay(delay,function()
                        local f=KResolveLocalFighter(); local item=f and f.EquippedItem
                        if item and item.StartShooting then pcall(item.StartShooting,item) end
                    end)
                end
                KFlick.State="Cooldown"; KFlick.Timer=KSettings.FlickbotCooldown/1000; return
            end
            local alpha=math.clamp((KFlick.ElapsedMs-cur.t)/math.max(nxt.t-cur.t,0.001),0,1); local dir=cur.direction:Lerp(nxt.direction,alpha)
            local cam=KWorkspace.CurrentCamera
            if cam then
                local pos=cam.CFrame.Position; cam.CFrame=CFrame.lookAt(pos,pos+dir)
            end
        elseif KFlick.State=="Cooldown" then
            KFlick.Timer=KFlick.Timer-dt; if KFlick.Timer<=0 then KFlick.State="Idle" end
        end
    end

    local KRageTab=Library:Tab("Kicia",10455604811)
    local KRageGroup=KRageTab:Group("Ragebot")
    KRageGroup:Toggle({Name="Enabled",Tooltip="启用 Kicia Ragebot",Callback=function(v) KSettings.Enabled=v; KController:SetEnabled(v) end})
    KRageGroup:Slider({Name="Stability",Min=0,Max=1.5,Default=0.15,Callback=function(v) KSettings.Stability=v end})
    KRageGroup:Slider({Name="Shoot Frames",Min=1,Max=5,Default=1,Unit="帧",Callback=function(v) KSettings.ShootFrames=v end})
    KRageGroup:Toggle({Name="Prioritize Hackers",Callback=function(v) KSettings.PrioritizeHackers=v end})
    KRageGroup:Toggle({Name="Use Primary",Callback=function(v) KSettings.WeaponPrimary=v end})
    KRageGroup:Toggle({Name="Use Secondary",Callback=function(v) KSettings.WeaponSecondary=v end})
    KRageGroup:Toggle({Name="Use Melee",Callback=function(v) KSettings.WeaponMelee=v end})
    KRageGroup:Dropdown({Name="On Empty",Options={"Swap","Reload","SwapOrReload"},Default="SwapOrReload",Callback=function(v) KSettings.OnEmpty=v end})
    KRageGroup:Dropdown({Name="Evasion Mode",Options={"Random","Translocate","ProjectileBreaker"},Default="Random",Callback=function(v) KSettings.EvasionMode=v end})
    KRageGroup:Slider({Name="Translocate Offset",Min=-5,Max=5,Default=-5,Callback=function(v) KSettings.TranslocateOffset=v end})
    KRageGroup:Slider({Name="Random Base Radius",Min=5,Max=100000000,Default=100,Callback=function(v) KSettings.RandomBaseRadius=v end})
    KRageGroup:Slider({Name="Random Range Factor",Min=0,Max=1,Default=0.5,Callback=function(v) KSettings.RandomRadiusFactor=v end})
    KRageGroup:Toggle({Name="Character Origin",Callback=function(v) KSettings.RandomAnchorFromCharacter=v end})

    -- Flickbot UI intentionally not added: the requested transfer is the left-side Ragebot feature.
    -- Flickbot implementation remains in the script for code preservation, but is disabled by default.


    KRunService.Heartbeat:Connect(function(dt)
